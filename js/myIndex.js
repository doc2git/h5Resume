...
var phoneRender = (function() {
	...
	return {
		init: function() {
			...
			listenMusic.play();
			//给listenTouch绑定接听事件，在移动端事件中click会有300ms的延迟 我们可以用移动端专门的原生事件 touchstart touchmove  touchend来进行模拟，zepto中给你做了处理封装好了一个方法 click（单击事件）
...
var messageRender = (function() {
	...
	function messageMove() {
		autoTimer = window.setInterval(function() {
			...
			//从第三条消息开始，整个消息框整体上移
			if(step >= 3) {
				bounceTop -= $cur[0].offsetHeight + 8;
//				if(step == 2) bounceTop -= 10;
...
//				var curOffsetHeight = $("#waiting-submit")[0].offsetHeight;
//				console.log(curOffsetHeight);
				$("#waiting-submit").css("display", "block");
//				$(".messageList").css("transform", "translateY("+ (curOffsetHeight + 10) + "px)").
...
//			console.log(279);
			if(index == example.activeIndex) {
				...
				console.log(item.id);
//				if(item.id == "page4") {
//					$("#page4").css("background-color", "#39f")
//				}
				return;
			}
			item.id = null;
